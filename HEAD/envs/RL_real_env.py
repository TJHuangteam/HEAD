...
SCENARIO_ENV_CONFIG = dict(
    # ===== Scenario Config =====
    data_directory=AssetLoader.file_path("nuplan_pittsburgh", unix_style=False),
...
class RLrealEnv(BaseEnv):
    @classmethod
    def default_config(cls):
        config = super(RLrealEnv, cls).default_config()
        config.update(SCENARIO_ENV_CONFIG)
        return config

    def __init__(self, config=None):
        super(RLrealEnv, self).__init__(config)
    ...
    def setup_engine(self):
        super(RLrealEnv, self).setup_engine()
