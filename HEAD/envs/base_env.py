...
from HEAD.algorithm.SAC.state_obs import LidarStateObservation
import time
...
class BaseEnv(gym.Env):
    ...
    def obs_function(self, object_id: str) -> Tuple[bool, Dict]:
        self.logger.warning("Obs function is not implemented. Return Done = False", extra={"log_once": True})
        return False, {}
    ...
    def _get_reset_return(self):
        ...
        for v_id, v in self.agents.items():
            ...
            obses[v_id] = self.observations[v_id].observe(v) ################################# todo:use self.obs_function to replace this
            ...
            _, obs_infos[v_id] = self.obs_function(v_id)
    ...
    def _get_step_return(self, actions, engine_info):
        ...
        for v_id, v in self.agents.items():
            ...
            obses[v_id] = o################################ todo: use self.obs_function to replace this
            _, obs_infos[v_id] = self.obs_function(v_id)
    ...
    def get_single_observation(self):
        if self.__class__ is BaseEnv:
            ...
            if self.config["agent_observation"]:
                ...
                o = ImageStateObservation(self.config) if img_obs else LidarStateObservation(self.config)##############todo: 想要改状态，改这个就行，把你想要的OBS继承Metadrive的父类，扔到里面就行
                ### todo: 必须在自己定义OBS的时候，继承Metadrive的父类
        return o
